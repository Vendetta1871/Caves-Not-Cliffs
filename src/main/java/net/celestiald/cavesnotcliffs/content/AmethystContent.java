...
public final class AmethystContent {
    ...
    public static void registerBlocks(RegistryEvent.Register<Block> event) {
        event.getRegistry().registerAll(
                ...
                new BlockTintedGlass()
                        ...
                        .setCreativeTab(CreativeTabs.BUILDING_BLOCKS),
                waterlogged("small_amethyst_bud", 3, 4, 1, false),
                waterlogged("medium_amethyst_bud", 4, 3, 2, false),
                waterlogged("large_amethyst_bud", 5, 3, 4, false),
                waterlogged("amethyst_cluster", 7, 3, 5, true));
    }
    ...
    private static Block waterlogged(String publicName, int height, int offset,
            int lightLevel, boolean cluster) {
        return BlockAmethystGrowth.waterloggedCompanion(publicName, height, offset,
                lightLevel, cluster).setRegistryName(id(
                        AmethystWaterlogging.companionPath(publicName)));
    }
    ...
    public static final class Models {
        ...
        public static void registerBlockColors(ColorHandlerEvent.Block event) {
            Block[] companions = new Block[AmethystWaterlogging.PUBLIC_STAGES.size()];
            for (int i = 0; i < companions.length; i++) {
                String path = AmethystWaterlogging.companionPath(
                        AmethystWaterlogging.PUBLIC_STAGES.get(i));
                companions[i] = ForgeRegistries.BLOCKS.getValue(id(path));
                if (!(companions[i] instanceof BlockAmethystGrowth)) {
                    throw new IllegalStateException(
                            "Waterlogged amethyst companion was not registered: " + path);
                }
            }
